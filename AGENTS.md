...
- 普通寒暄、学习建议咨询、澄清问题和已保存设置查询必须作为普通聊天处理，主会话仍必须调用当前模型并写入 `model_calls`，不得用程序硬编码回复代替模型；用户询问学习设置、当前模型、已开启权限、目标、基础、当前考试、考试时间或学习计划依据时，必须优先读取已保存上下文直接回答，字段为空才反问；只有明确词表、截图词表、显式“出题 / 练习 / 刷题 / 考我”等学习动作才进入正式组卷流程，只有明确打开、修改、配置、添加、保存或导入设置的动作才进入设置流程。

...
- 普通寒暄、学习建议咨询和澄清问题必须作为普通聊天处理，主会话仍必须调用当前模型并写入 `model_calls`，不得用程序硬编码回复代替模型；普通聊天不得自动生成题组或写入题目，只有明确词表、截图词表、显式“出题 / 练习 / 刷题 / 考我”等学习动作才进入正式组卷流程。主会话和分支会话必须把用户画像作为一等上下文：用户询问目标、基础、当前考试、考试时间或学习计划依据时，必须优先读取已保存的学习目标、学习背景、考试名、目标语言、考试时间和每日学习时长，字段为空才反问。

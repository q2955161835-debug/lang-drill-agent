...
截图词表导入流程：右侧工作台粘贴 OCR（文字识别）词表文本后，点击“导入并开始练习”，后端会新建截图词表练习会话、写入本次词表、自动生成完整题组并展示第 1 题。主聊天栏粘贴 3 个以上截图词条也会复用同一导入流程。词表可使用“单词独占一行 + 下一行释义”、`word n. 释义` 或 `word: 释义` 格式。题目必须是考试式语境题，例如英文句子空缺、阅读语境选择或同义改写，不能只是中文释义匹配。

...
- 快速开始：长期学习总面板底部提供“导入截图 / 快速开始”。当天没有导入或题组时打开右侧截图导入；当天已有内容时自动发送“继续今天的答题”。
- 主聊天截图导入：用户把多行截图词表直接粘贴到主聊天栏时，后端复用侧边栏截图导入流程，自动创建截图练习会话、导入词表并生成题组。
...
截图词表导入流程：右侧工作台粘贴 OCR（文字识别）词表文本后，点击“导入并开始练习”，后端会新建截图词表练习会话、写入本次词表、自动生成完整题组并展示第 1 题。主聊天栏粘贴多行截图词表也会复用同一导入流程。题目必须是考试式语境题，例如英文句子空缺、阅读语境选择或同义改写，不能只是中文释义匹配。

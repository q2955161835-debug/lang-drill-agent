...
Both a Web edition and a Windows desktop edition are provided. The desktop edition wraps the same React/Vite UI with Tauri and starts a local FastAPI backend; the Web development workflow stays unchanged. The current version is `v1.0.0-experimental.1`. The default learning flow remains unchanged and creative mode is an optional experimental capability.

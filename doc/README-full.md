...
> 本文档是长版完整维护说明；面向公开展示和招聘快速阅读的短版入口在根目录 `README.md`。
...
当前公开定位重点面向英语四级/六级（CET-4/CET-6，大学英语四级/六级）、法语四级（CFT-4，大学法语四级）与日语四级/六级（CJT4/CJT6，大学日语四级/六级）备考：围绕考纲词汇、语法范围、阅读/翻译/写作题型、真题试卷资产、错题回流和间隔复习，把“每天刷什么、怎么判题、错题何时回来”沉淀为可追踪的学习流程。听力题型当前仅预留入口，未接入听力题和语音模型前默认关闭且不可勾选。
...
- 聊天式学习入口：像常见 AI（人工智能）聊天工具一样输入普通寒暄、学习建议、今日学习内容、答案或追问；普通聊天仍调用当前模型并记录模型调用，但不会自动生成题组，只有明确词表或显式“出题 / 练习 / 刷题 / 考我”等学习动作才进入组卷。
...
- 判题讲解与总结：Evaluator Tutor（判题讲解 Agent）负责复杂题型判分、错误诊断和讲解；当日总结由当前模型基于数据库中的当日会话、题目、作答、正确答案、知识标签和最近聊天生成详细复盘。
- 分支对话：拖选文本或右键消息后开启右侧分支小窗，创建分支和后续分支消息都调用当前模型，共享必要主上下文，默认不写回主会话。
...
- Agent 设置权限：设置页提供“权限”页签；截图导入、学习数据库写入、历年真题草稿、联网功能、考试目标和上下文容量等非敏感权限默认开启；拓展 Skills（拓展技能）不进入权限页总权限，由“拓展 Skills”页单项开关控制，Multi Search Engine（多搜索引擎）默认启用，其它拓展 Skill 默认关闭；模型配置、配置自定义模型、API Key（接口密钥）、MinerU token、数据迁移等敏感设置默认关闭，开启后关键保存仍由用户手动确认。
- 拓展 Skills（拓展技能）：设置页提供“拓展 Skills”页签，分开展示始终开启的内置联网检索和可选拓展 Skills；内置联网检索工具始终可用，实际联网调用仍受“联网功能”权限控制；拓展 Skills 不再有总权限，Multi Search Engine（多搜索引擎）默认启用用于生成可审计搜索入口，其它拓展 Skill 通过单项开关独立启用。
...
负责复杂题型判分、错误诊断、讲解生成、错题归因和学习反馈。简单选择题可由程序直接判定，减少无意义 token（令牌）消耗；当日总结作为独立 summary（总结）任务调用当前模型，依据数据库明细生成复盘。
...
9. 用户输入“总结 / 复盘”后，当前模型读取当日完整学习明细生成详细总结，并给出错题归因和下一步复习建议。
...
- 主题与气泡：页面背景保留原有浅色渐变纹理和深色背景图；主聊天与分支消息气泡使用浅蓝紫/深蓝紫专用颜色，不复用纯白 surface（表面色）或近黑 text（文本色）。
...
- 权限：控制会话 Agent 可触发的程序能力；非敏感权限默认开启，敏感设置权限默认关闭且集中展示，授权后仍只填入草稿，所有敏感保存动作需要用户确认。
- 拓展 Skills：分开展示始终开启的内置联网检索、默认启用的 Multi Search Engine（多搜索引擎）、拓展 Skills（拓展技能）根目录、已安装拓展技能、无密钥拓展技能数量和单个拓展 Skill 启用/关闭开关；内置联网检索不依赖拓展 Skills 开关。
...
frontend/public/assets/         前端公开静态资源；包含深色主题背景图等资产

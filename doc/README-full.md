...
- Agent 设置权限：设置页提供“权限”页签；截图导入、学习数据库写入、历年真题草稿、联网功能、考试目标和上下文容量等非敏感权限默认开启；拓展 Skills（拓展技能）不进入权限页总权限，由“拓展 Skills”页单项开关控制且默认关闭；模型配置、配置自定义模型、API Key（接口密钥）、MinerU token、数据迁移等敏感设置默认关闭，开启后关键保存仍由用户手动确认。
- 拓展 Skills（拓展技能）：设置页提供“拓展 Skills”页签，分开展示始终开启的内置联网检索和可选拓展 Skills；内置联网检索工具始终可用，实际联网调用仍受“联网功能”权限控制；拓展 Skills 不再有总权限，每个拓展 Skill 默认关闭并通过单项开关独立启用。
...
- 拓展 Skills：分开展示始终开启的内置联网检索、拓展 Skills（拓展技能）根目录、已安装拓展技能、无密钥拓展技能数量和单个拓展 Skill 启用/关闭开关；内置联网检索不依赖拓展 Skills 开关。
